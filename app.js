  ...
  fetch(GAS_URL + "?action=getConfig")
    ...
    .catch(() => {
      document.getElementById("configLoader").innerHTML =
        '<p style="color:#dc2626">❌ Config load nahi hua. GAS URL check karo.</p>';
    });
}
...
function renderDashboard(logs) {
  ...
  if (Object.keys(workerMap).length === 0) {
    ...
    wHtml = `<table class="worker-table">
      <thead><tr><th>Worker</th><th>Hall</th><th>Stage Breakdown</th><th>Total</th></tr></thead>
      <tbody>`;
  ...
  let html = `<table class="log-table">
    <thead><tr><th>Worker</th><th>Hall</th><th>Stage</th><th>Qty</th></tr></thead>
    <tbody>`;
  logs.forEach(r => {
